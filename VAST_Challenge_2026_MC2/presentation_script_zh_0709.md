# MC2 中文答辩脚本（内部 5 分钟版本）

建议时长：5 分钟。本文件用于内部答辩或排练，不是官方不超过 4 分钟的视频脚本；正式视频请使用 `video_script_4min_zh.md`。答辩时优先打开 `rebuild/index.html` 展示交互网页；如果时间紧，打开 `slides_0709.html` 快速讲完。
