...
> **Exploratory notes only / 非最终结论声明：** 本文件保留早期侦查过程和假设推演，不作为最终官方答案。最终可提交结论以 `final_report_0709.html`、`critical_review.md` 和 `rebuild/` 为准。本文中关于攻击者身份、人物动机、知情程度、后门植入、配置篡改、whistleblowing 等表述均应视为探索性假设；除非在最终报告中被明确标注为 observed，否则不得作为事实提交。
